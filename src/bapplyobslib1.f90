   ...
   do ikeep = 1,nkeep
      ...
      if(subtract_enabled)then
         ...
         if(nfragments > 1) then ! this nfragments reference is ok
            dot8a = dot8
            call BMPI_ALLREDUCE(dot8a, dot8, 1, MPI_SUM, icomm, ierr)
		   end if
...
!--------- REDUCE-----
  ...
  if(nproc > 1)then
     call BMPI_Allreduce(xj2,xj2tmp,1,MPI_SUM,icomm,ierr)
     call BMPI_Allreduce(xt2,xt2tmp,1,MPI_SUM,icomm,ierr)
     xj2 = xj2tmp
     xt2 = xt2tmp
  end if
...
!--------- REDUCE-----
  ...
  if(nproc > 1)then
     call BMPI_Allreduce(xj2,xj2tmp,1,MPI_SUM,icomm,ierr)   ! note here xj2 and xt2 are just dummy variables
     call BMPI_Allreduce(xt2,xt2tmp,1,MPI_SUM,icomm,ierr)
     xj2 = xj2tmp
     xt2 = xt2tmp
  end if
