...
!
!=================================================================== 
subroutine master_readin_tbmes(J2only)
  use io
  use nodeinfo
  use sporbit
...
#ifdef _MPI
  call BMPI_Bcast(emptyHam,1,0,MPI_COMM_WORLD,ierr) 
#endif 
...
!........ SKIP IF ANY ORBITAL IS STERILE W = 99.....

	if(orbqn(1,ia)%w==99 .and. orbqn(2,ia)%w==99 )cycle
	if(orbqn(1,ib)%w==99 .and. orbqn(2,ib)%w==99 )cycle
	if(orbqn(1,ic)%w==99 .and. orbqn(2,ic)%w==99 )cycle
	if(orbqn(1,id)%w==99 .and. orbqn(2,id)%w==99 )cycle
...
!........... WARNING MESSAGES.... 
  if(maxsp < maxorblabel)then
     if(iproc==0)then
      print*,' '
      print*,' WARNING ',maxsp,maxorblabel
...
!	  print*,' SETTING DUPLIFLAG ',dupliflag
  end if
...
!-------------------- PP interaction ------------------------- 
     ...
     c = ic  
     d = id 
     if(orbqn(1,ia)%w==99 .or. orbqn(1,ib)%w==99 )cycle		
     if(orbqn(1,ic)%w==99 .or. orbqn(1,id)%w==99 )cycle		
...
! print*,i,' reading NN '
    ...
    if(ic <= neutron_offset .or. id <= neutron_offset)then
     ...
     if(orbqn(2,a)%w==99 .or. orbqn(2,b)%w==99 )cycle		
     if(orbqn(2,c)%w==99 .or. orbqn(2,d)%w==99 )cycle		
...
!     if(orbqn(1,a)%w==99 .or. orbqn(2,b)%w==99 )cycle		
!     if(orbqn(1,c)%w==99 .or. orbqn(2,d)%w==99 )cycle		
...
!-------- NO PHASE, ALREADY IN CORRECT ORDER 
     ...
     indx = (pair1-iref)*(pair1-1-iref)/2+pair2-iref+istart    
!	 if(indx==650)print*,' YO ',a,b,c,d,j,t,vv,indx
...
!		write(51,*)pnme(indx)%v(j),factor,dupliflag
!	 else
...
!			  print*,' INDEX ',cpair,dpair,indx
		  ...
		  if(.not.dens2bflag)then
             xxme(indx)%jmin = jmin 
             xxme(indx)%jmax = jmax 
			 totpar = (-1)**(orbqn(it,a)%l + orbqn(it,b)%l + orbqn(it,c)%l + orbqn(it,d)%l)
             if(jmax >= jmin) then 
                allocate( xxme(indx)%v(jmin:jmax), stat=aerr )  
                if(aerr /= 0) call memerror("set_tbme_XXarray 5") 
                xxme(indx)%v(:) = 0.0 	
  			    if(a==b .or. c==d)then
  				   if(totpar==1)nv2bme_tot = nv2bme_tot + (jmax+1)/2
  			    else
  				   if(totpar==1)nv2bme_tot = nv2bme_tot + jmax - jmin+1
                end if	
             endif      
	      else
	          jabmin = abs(ja-jb)/2
...
!.................... COMPUTE # OF MATRIX ELEMENTS 
  ...
  do dpair = 1,ncouplesPN 
     ...
     do cpair = 1, cpairend
		  ...
		  totpar = (-1)**(orbqn(1,a)%l + orbqn(2,b)%l + orbqn(1,c)%l + orbqn(2,d)%l)
...
#ifdef _MPI
        call BMPI_BCAST(pspe(:),size(pspe),0,MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(nspe(:),size(nspe),0,MPI_COMM_WORLD,ierr) 
#endif
...
#ifdef _MPI
        call BMPI_Bcast(num_me, 1,  0, MPI_COMM_WORLD, ierr) 
#endif 
...
#ifdef _MPI
        call BMPI_Bcast(indx1, num_me,  0, MPI_COMM_WORLD, ierr) 
        call BMPI_Bcast(indx2, num_me,  0, MPI_COMM_WORLD, ierr) 
        call BMPI_Bcast(xxme, num_me,  0, MPI_COMM_WORLD, ierr) 
#endif 
        if(iproc > 0)then 
           do ii = 1, num_me 
              i = indx1(ii) 
