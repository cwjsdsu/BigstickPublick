...
!................. LANCZOS .........................
  ...
  integer :: itskipeig_def = 10    ! how often to print out intermediate energies during Lanczos DEFAULT
  logical :: skipeig = .true.  ! if TRUE then only check eigenvalues every ITSKIPEIG times
  logical :: large_skip_enabled = .true.  ! enables skipping more diagonalizations ADDED 7.10.7
  real    :: frac_large_skip = 0.01  ! if large_skip_enabled, what fraction of maxiter to diagonalize
...
!       ...added 7.11.2 to speed up lanczos
  logical :: use_switch_esolver = .true. ! flag to switch to faster eigensolvers
  integer :: nswitch_esolver = 100 ! if nkeep > nswitch_esolver, and use_switch_esolver, use alternate
  logical :: use_test_converge = .true.  ! use a restricted set of eigenvalues
...
!   real    :: maxjumpmemory_default  = 32.5    ! in Gb
   real    :: maxjumpmemory_default  = 128.0    ! in Gb  KSM - for DM runs on Edison
...
!.............. BREAKING LANCZOS VECTORS FOR HAMILTONIAN MAT-VEC MULTIPLY............
   ...
   integer(8) :: maxfragmentsize_default = 500000000 !changed in 7.9.12   
...
!   logical :: annex_bundles = .false. ! if TRUE, then seek to annex bordering bundles which share jumps ADDED 7.8.0
   ...
   logical  :: iffulluseph  = .false.    ! if TRUE, when a shell is filled, automatically turns to p-h conjugation
   integer(4) :: fulltrigger = 10  ! only use iffulluseph if number of particles > fulltrigger; 
