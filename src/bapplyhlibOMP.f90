...
!  if (iproc == 0) verbose_ompham = .true.
  ...
  if (verbose_ompham) print *, "MPI Time (getLoadDist start) : ", Get_Wtime() - mpiStartTime
  nn = 240
  count = 0
...
!	 if(opbundle(ibundle)%annexed)cycle
     ...
     case default
        print *, "wrong opbundle type (LoadDist 1)", opbundle(ibundle)%optype
        stop 30
...
!$omp end parallel do
#ifdef _MPI
  if (verbose_ompham) print *, "MPI Time (range estimation) : ", get_Wtime() - mpiStartTime
#endif
...
!$omp end parallel do
  ...
  if (verbose_ompham) print *, "MPI Time (bucket) : ", get_Wtime() - mpiStartTime
...
105 format('DIST1', i6, a4, 2i12, i6, a7, 257i12)
  ...
  if (verbose_ompham) print *, "MPI Time (basis) : ", get_Wtime() - mpiStartTime
!    call MPI_Barrier(MPI_COMM_WORLD, ierr)
...
114 format("Bundle ", i6, i6, ' pnb ', 2i12, ' offset ', 4i12, 4i12)
       ...
       case default
          print *, "wrong opbundle type (LoadDist) ", opbundle(ibundle)%optype
          stop 30
...
!$omp end parallel do
    ...
    if(verbose_ompham)then      
       ...
       if (verbose_ompham) print *, "MPI Time (getLoadDist finish) : ", get_Wtime() - mpiStartTime
   end if
   return
