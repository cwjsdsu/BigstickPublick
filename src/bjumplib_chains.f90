...
!
! MODIFED in 7.10.6 to pre-check if there are overlapping blocks; not clear it speeds up much, however
...
integer(8) :: descentstop
 ...
 integer, allocatable :: rblocklist(:),lblocklist(:)
 logical, allocatable :: okblock(:)
 logical :: good2go
 integer :: blockcount
 integer :: nblocksall
 ...
 do iblocki = 1,nblocki
	 nblocksall =nblocksall+isdescent%block(iblocki)%ndescendents
 end do
 
 allocate(rblocklist(nblocksall),lblocklist(nblocksall),okblock(nblocksall))
 blockcount = 0
 rblocklist = 0
 lblocklist = 0
 do iblocki = 1,nblocki
    do idescend = 1,isdescent%block(iblocki)%ndescendents
		...
		if(blockcount > nblocksall)then
			print*,' misestimated block counts '
			stop
		end if
		lblocklist(blockcount)=isdescent%block(iblocki)%flblock(idescend)
		rblocklist(blockcount)=isdescent%block(iblocki)%frblock(idescend)
	end do
 end do
 good2go = .false.
 okblock = .false.
 do i = 1,blockcount
     do iblockf = 1,nblockf
	     do fdescend=1,fsdescent%block(iblockf)%ndescendents
			 if(lblocklist(i)==fsdescent%block(iblockf)%flblock(fdescend) .and. & 
			  rblocklist(i)==fsdescent%block(iblockf)%frblock(fdescend))then
			     good2go = .true.
				 okblock(i)=.true.
			     exit
		     end if
 ...
 curgen => geneology
 if(.not.good2go)then
 	deallocate(okblock,rblocklist,lblocklist)
 	return
 endif
 blockcount = 0
 do iblocki = 1,nblocki
    ...
    do idescend = 1,isdescent%block(iblocki)%ndescendents
		blockcount = blockcount +1
		if(.not.okblock(blockcount))cycle
...
!==============================================================
subroutine matchmaker2(it,igen,is,isdescent,fgen,fs,fsdescent,ordered,curgen,ngeneologies)
...
integer(8) :: descentstop
 ...
 do iblocki = 1,nblocki
    do idescend = 1,isdescent%block(iblocki)%ndescendents
		blockcount =blockcount+1
		if(blockcount > nblocksall)then
			print*,' misestimated block counts ',nblocksall,blockcount
			stop
		end if
		lblocklist(blockcount)=isdescent%block(iblocki)%flblock(idescend)
		rblocklist(blockcount)=isdescent%block(iblocki)%frblock(idescend)
    ...
    do idescend = 1,isdescent%block(iblocki)%ndescendents
      ...
      do iblockf = 1,fstop
         ...
         do fdescend = 1,descentstop
             ...
             allocate(curgen%gen(igen+fgen), stat=aerr)
             if(aerr /= 0) call memerror("matchmaker2 1")
             do i = 1,igen
               curgen%gen(i) = isdescent%block(iblocki)%descent(i,idescend)
             enddo  !i
             do i = 1,fgen,1
...
!------------------------ SET UP FOR NEXT
             if(.not.associated(curgen%next))then
                allocate(curgen%next, stat=aerr)
                if(aerr /= 0) call memerror("matchmaker2 2")
                 nullify(curgen%next%next)
             end if
             curgen => curgen%next
         enddo ! fdescend
