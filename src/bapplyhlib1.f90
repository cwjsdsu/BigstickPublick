...
!			print*,' PP ',statei,statef,xme,Xoplabel,xjmp
	     end do  ! csd
...
!---------- GET INITIAL, FINAL SDs and place in basis..............
	...
	!					print*,' HEY ',xme
	!					write(34,*)xme,ibundle,csd,xjmp,Xoplabel
                        vecout(statef) = vecout(statef) +  xme*vecin(statei)
                     end do  ! xjmp
                  end do  ! csd
...
!------------------------------------------------------------
  ...
  integer :: a,b,c,d
  integer(kind=8) :: coplabel,doplabel
  integer :: phasep,phasen,pairphase
  real(kind=4) ::   xme
  integer(kind=basis_prec) :: statei, statef
  integer(kind=4) num_threads
...
!$omp parallel do private(mythread,startp_thread,npjmps_thread)           &
!$omp          private(pjmp,njmp,psdi,psdf,nsdi,nsdf,phasep,phasen,a,b,c,d)   &
!$omp          private(coplabel,doplabel,xme,statei,statef,pairphase)                   &
!$omp          firstprivate(njmpstart, njmpend)       &
!$omp          shared(p1b_isd,p1b_fsd,p1b_phase,p1b_cop,p1b_dop)              &
!$omp          shared(ibundle,opbundle)    &
!$omp          shared(n1b_isd,n1b_fsd,n1b_phase,n1b_cop,n1b_dop)              &
!$omp          shared(cpnpair,dpnpair,phasepnpair,vecin,vecout)
     do mythread = 0,numpthreads -1
...
!----------- FIND MATRIX ELEMTN --------------------------------------------
           ...
           d = n1b_dop(njmp)  ! KSM: Neutron 1-body destruction label
           phasen = n1b_phase(njmp)
		   pairphase=phasepnpair(b,a)*phasepnpair(d,c)
           coplabel = cpnpair(b,a)
           doplabel = dpnpair(d,c)
           xme = hmatpn(coplabel + doplabel)   ! get matrix element
           xme = xme*phasep*phasen*pairphase             ! multiply matrix element by jump phases
           nsdi = n1b_isd(njmp)
           nsdf = n1b_fsd(njmp)
           statei = nsdi + psdi                ! initial state in combined basis
           statef = nsdf + psdf                ! final state in combined basis
  ...
  do mythread = 0, numnthreads-1
     ...
     do njmp = startn_thread + 1, startn_thread + nnjmps_thread     
        ...
        do pjmp = pjmpstart,pjmpend
           ...
           a  = p1b_cop(pjmp) 
           c  = p1b_dop(pjmp)
           phasep = p1b_phase(pjmp)
		   pairphase=phasepnpair(b,a)*phasepnpair(d,c)   ! phase of proton jumps
!--------- LOOP OVER NEUTRON JUMPS -----------------------------------------
!----------- FIND MATRIX ELEMENT -------------------------------------------
           coplabel = cpnpair(b,a)
           doplabel = dpnpair(d,c)		   
...
! 
!
subroutine applyhPNbundledTR_g_old (vchar,hchar,startbundle,endbundle )
  use localvectors
  use nodeinfo
  use system_parameters
  use jumpNbody
...
!$omp end parallel do
  ...
  return
end subroutine applyhPNbundledTR_g_old
...
!----------- FIND MATRIX ELEMENT ---------------------------
           ...
           statef = nsdf + psdf                ! final state in combined basis
           voutp(statef) = voutp(statef) + xme*vecin(statei)
!		  if(statei==statef)print*,' me (1)1',statei,xme,a,bb,c,dd,psdi,nsdi,pjmp,njmp,coplabel,doplabel
!		  if(statei==2 .and. statef==2)print*,' me (1)2',xme,a,bb,c,dd
...
!----------- FIND MATRIX ELEMENT ------------------------------
           ...
           statef = nsdf + psdf                  ! final state in combined basis
           voutp(statei) = voutp(statei) + xme*vecin(statef)
!		   print*,' Miss me?',statei,statef,xme,pjmp,njmp
        end do  ! pjmp
     end do  ! njmp
...
!----------- FIND MATRIX ELEMENT -------------------------------------------
              ...
              statef = nsdf + psdf                  ! final state in combined basis
              voutp(statei) = voutp(statei) + xme*vecin(statef)
!			  if(statei==1 .and. statef==1)print*,' me (2)',xme
           end do
      end do  ! pjmp
     end if
  end do
