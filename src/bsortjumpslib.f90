...
!   use distrinfo
   ...
   integer  :: sortrange = 100   ! empirical parameter for chunking searches
...
!				   if(opbundle(ibundle)%hchar/='b')then
				   ...
				   call sort1bjumps(npjmps,p1b_isd(startp+1),p1b_fsd(startp+1),              &
                       p1b_cop(startp+1),p1b_dop(startp+1),    p1b_phase(startp+1))
!.... secondary sort intended to improve locality	
!     not yet clear it helps, and the sorting is slow.....				   
!				   call secondsort1bjumps(npjmps,p1b_isd(startp+1),p1b_fsd(startp+1),              &
!                      p1b_cop(startp+1),p1b_dop(startp+1),    p1b_phase(startp+1),sortrange)
...
! 
                ...
                call sort1bjumps(nnjmps,n1b_fsd(startn+1),n1b_isd(startn+1),              &
                       n1b_cop(startn+1),n1b_dop(startn+1),     n1b_phase(startn+1))   
!	            call secondsort1bjumps(nnjmps,n1b_fsd(startn+1),n1b_isd(startn+1),              &
!	                          n1b_cop(startn+1),n1b_dop(startn+1),     n1b_phase(startn+1),sortrange)  
...
!==============================================================
! secondary sorting -- added 7.10.1  July 2021
! all jumps have two sets of SDs.  After sorting one of them,
! sort the second within a given sortrange. 
! What this means is: on the already-sorted set, for sets within a 
! given range, sort on the other set within that range
!
! here assume it is sorted on x1b_fsd already
!
! current version runs slow; will need to speed up to make viable
!
subroutine secondsort1bjumps(ndim,x1b_isd,x1b_fsd,x1b_cop,x1b_dop,x1b_phase,sortrange)
   use nodeinfo
   use precisions
   implicit none
   ! arguments
   ...
   integer(kind=8) :: sortstart,sortend,i
   integer(kind=basis_prec) :: fsd_ref
   logical searching
   
   if(sortrange < 1)then
	   print*,' problem in secondsort1bjumps ',sortrange
	   stop
   end if
   
   sortstart = 1
   sortend = 1
   do while(sortstart <= ndim)
	   
!.... FIND LIMITS OF SEARCH, SORTSTART AND SORTEND.....	   
	   fsd_ref = x1b_fsd(sortstart)
	   sortend = sortstart
	   searching = .true.
	   do while(searching)
		   if(sortend+1 <= ndim)then
			   if(x1b_fsd(sortend+1) < fsd_ref+sortrange)then
				   sortend=sortend+1
			   else
				   searching=.false.
			   end if
	   ...
	   if(sortend==sortstart)then
	   	sortstart= sortstart+1
	   	if(sortstart > ndim)exit
		cycle
	   end if
!........... NOW SORT WITHIN; try to use existing code	   
print*,' advanced sorting ',sortend-sortstart+1
call sort1bjumps(sortend-sortstart+1,x1b_fsd(sortstart),x1b_isd(sortstart),              &
    x1b_cop(sortstart),x1b_dop(sortstart),x1b_phase(sortstart))
...
end subroutine secondsort1bjumps
