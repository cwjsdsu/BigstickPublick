...
!  integer, pointer     :: hsd(:,:)
  ...
  integer :: aerr
  logical :: foundit
  integer :: ilist,nword0,iword0
  integer, allocatable :: countbhops(:)
  it = abs(ith) ! nspecies

  nhblocks = hblock(ith)%nhblocks
  allocate(countbhops(nhblocks))
...
!------------------- FIRST DETERMINE THE NUMBER OF BLOCK HOPS
  ...
  ndjumps = 0
  countbhops = 0
!$OMP PARALLEL DO private(nh,nhaiku,occcom,i,iarray,occgroup,ndbhops)
!!$OMP PARALLEL DO private(nh,nhaiku,i,ndbhops)
...
!----search through all haikus and determine common occupied states --------
     do i = 1, nhaiku
        occcom(:) = ior(occcom(:),hblock(ith)%list(iblock)%hsd(:,i))
     end do
...
!--------- DETERMINE OCCUPIED GROUPS ---------------------------
	 ...
	 countbhops(iblock)=ndbhops
  enddo ! iblock
  
  ndbhops = 0
  do iblock = 1,nhblocks
	  n = countbhops(iblock)
	  countbhops(iblock)=ndbhops
	  ndbhops = ndbhops+n
...
!---------- LOOP OVER BLOCK HOPS AND CREATE HOPS ------------
  ...
  n = 0
  ndjumps = 0
!$OMP PARALLEL DO private(n,occcom,iarray,occgroup,groupmap,fmap), SCHEDULE(dynamic),reduction(+:ndjumps)  
  do iblock = 1, nhblocks  ! loop over all blocks
	  n = countbhops(iblock)
	  call shiva_sub(n,ith,iblock,nword(it),occcom,iarray,occgroup,groupmap,fmap,ndjumps)
  enddo ! iblock
  ...
  deallocate ( occcom, iarray, occgroup, groupmap, fmap,countbhops ) ! Free memory
  return
end subroutine shiva
!=============================================================
subroutine shiva_sub(n,ith,iblock,nw0,occcom,iarray,occgroup,groupmap,fmap,ndjumps)
    use system_parameters
    use verbosity
    use haiku_info
    use haikus
    ...
    integer, allocatable     :: hsd(:,:)
!    integer, pointer     :: htmp(:,:)
    integer, pointer     :: hsdi(:)
    integer :: occcom(nw0)
    integer :: iarray(nhspsmax)
  
    logical :: occgroup(ngroups(ith))
    integer :: groupmap(nhspsmax),fmap(nhspsmax)
...
!    integer, allocatable :: countbhops(:)
    ...
    it = abs(ith) ! nspecies
     ...
     nh      = hblock(ith)%list(iblock)%nh
     if ( nh > maxNh(ith) .or. nh == 0 ) return

     nhaiku  = hblock(ith)%list(iblock)%nhsd
	 allocate(hsd(nword(abs(ith)),nhaiku))
	 hsd = hblock(ith)%list(iblock)%hsd
!     hsd    => hblock(ith)%list(iblock)%hsd(:,:)
     occcom  = 0
!----search through all haikus and determine common occupied states --------
...
!----LOOP OVER HAIKUS AND COUNT UP HOPS  --------
     do i = 1, nhaiku
!        call convertbin(ith,hblock(ith)%list(iblock)%hsd(:,i),iarray)
...
!        extraphase = (-1)**(np(it) - nh )
        ...
        do iadd = 1, nhaiku
!           if ( iand(imask,hblock(ith)%list(iblock)%hsd(iword,iadd)) /=0 ) then
	       if ( iand(imask,hsd(iword,iadd)) /=0 ) then
              bunny(ith)%dblockhop(m)%nhops = bunny(ith)%dblockhop(m)%nhops+1
              nhops = bunny(ith)%dblockhop(m)%nhops
!---------------
              hsdi => hblock(ith)%list(iblock)%hsd(:,iadd)
...
!              hblock(ith)%list(iblock)%hsd(iword,iadd) = hblock(ith)%list(iblock)%hsd(iword,iadd)-imask
...
!              call search_haiku(ith,hsdi,ntmp,htmp,fadd)
			  ...
			  do ilist = 1, ntmp
			     iword0 = 1
!			     do while ( hblock(ith)%list(iblock)%hsd(iword0,iadd) == hblock(ith)%list(fblock)%hsd(iword0,ilist) )
			     do while ( hsd(iword0,iadd) == hblock(ith)%list(fblock)%hsd(iword0,ilist) )
			        if ( iword0 == nword0 ) then
						foundit=.true.
						fadd = ilist
						exit
					endif
			  ...
			  if(.not.foundit)then
			     print*, ' did not find haiku in list! (sub!)'
			     print* ,hsd(:,iadd),ntmp,nword(abs(ith))
			     do ilist = 1, ntmp
			         print*, hblock(ith)%list(fblock)%hsd(:,ilist)
			     end do
			     stop 919
		       endif
...
!              hblock(ith)%list(iblock)%hsd(iword,iadd) = hblock(ith)%list(iblock)%hsd(iword,iadd)+imask
	 ...
	 deallocate(hsd)
	return
end subroutine shiva_sub
!===========================================================================
subroutine shiva2(ith)
...
!              call search_haiku(ith,hsdi,ntmp,htmp,fadd)
			  ...
			  if(.not.foundit)then
			     print*, ' did not find haiku in list! (new)'
			     print* ,hblock(ith)%list(iblock)%hsd(:,iadd),ntmp,nword(abs(ith))
			     do ilist = 1, ntmp
			         print*, hblock(ith)%list(fblock)%hsd(:,ilist)
			     end do
			     stop 919
...
!----Replace particle-------------------------------------------------------
  ...
  deallocate ( occcom, iarray, occgroup, groupmap, fmap,countbhops ) ! Free memory
  return
end subroutine shiva2
!=============================================================
subroutine shiva_old(ith)
