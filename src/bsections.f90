...
subroutine section_hsp(it)
  ...
  integer :: i
  integer :: n
  integer :: nlastw ! added to avoid W = 99
...
!------------------COUNT UP # OF SECTIONS-----------------------------------
  ...
  do i = 2, nhsps_eff(it) ! nhsps(it)
	 if(hspsqn(it,i)%w==99)exit ! added to make w = 99 sterile
...
!------------------Find START, FIN of each section--------------------------
  ...
  section(it)%list(n)%fin = nhsps_eff(it) ! nhsps(it)
...
!  print*,' section w ',section(it)%list(:)%w
