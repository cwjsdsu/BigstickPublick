   ...
   if(.not.threebody .and. menu_char /= 'dx'.and. menu_char /= 'o')then     
     ...
     do it = 1,2
        if(np(it) > 1)then	 
         ...
         call set2bsectorjumps(it,.false.,  hermflag, whermflag2p ,change2bodyqs, .true., 2 )
         call set2bsectorjumps(it,.true.,  hermflag,whermflag2p  ,change2bodyqs,.true., 2 )
         call masterfindconjumps2b(it,.false.)     ! NOTE: Need to find conjugate jumps because if none, then skip
         call master2bodyjumps(it,.false.  )
		 call split_sectorjumps(2,it)
		 call masterfindconjumps2b(it,.true.) 
...
! NOTE that I use "hermiticity" on 1-body jumps but not "W-hermiticity"
        ...
        do it = 1,2	
           call master1bodyjumps(it,.false.)
		   call split_sectorjumps(1,it)
        end do		
...
!----------- FLAGS FOR RESTRICTION ON CHANGING q#s in jumps-----------------
   ...
   if(.not. ham_readin .and. menu_char /= '2b' .and. menu_char /= 'jp')then
...
! OPENMP added in 7.10.1
!$omp parallel do private(isjmp,shiftjump,startjump,endjump,is,fs,ngeneologies,njumps,curgen,igen) & 
!$omp  shared(x2bjump,it,descent,nbody,fill,restrictjumps,compactjumpstorage)  &
!$omp schedule(static)
...
!.......... SKIP IF NO CONJUGATE JUMPS...........
     ...
     curgen =>geneology
     call matchmaker2(it,nbody,is,descent%sector(is),nbody,fs,descent%sector(fs),.false., curgen,ngeneologies)
...
!$omp end parallel do
  ...
  if(.not.fill)then
     if(it == 1)then
        ...
        if ( iproc == 0  ) write(logfile,*)totjumps,' proton 1-b jumps' 
		
     else
        totn1bjumps = totjumps
        if(iproc==0 .and. verbose_jumps)write(6,*)totjumps,' neutron jumps ' 
        if ( iproc == 0  ) write(logfile,*)totjumps,' neutron 1-b jumps' 
...
!			print*,iproc,' ALLOCAMATING ',minjump,maxjump			
           ...
           if(aerr /= 0)then
			   print*,iproc,' reserve error 30 ',minjump,maxjump
			    call memerror("master2bodyjumps 30")
			end if
        end if
        if ( .not.allocated( n2b_fsd ) ) then
           allocate(n2b_fsd( minjump:maxjump), stat=aerr)
           if(aerr /= 0) then
			   print*,iproc,' reserve error 31 ',minjump,maxjump
			   call memerror("master2bodyjumps 31")
		   end if
        end if
        ...
        if(threebody)then
           ...
           if ( storeXXmesjumps) then
             ...
             if ( .not.allocated( n2b_op ) )  then
              ...
              if(aerr /= 0)then
			   print*,iproc,' reserve error 53 ',minjump,maxjump
			    call memerror("master2bodyjumps 53")
			end if
             end if
           end if
        endif
...
!OPENMP added 7.10.1
!$omp parallel do private(isjmp,shiftjump,startjump,endjump,is,fs,ngeneologies,njumps,curgen,igen) & 
!$omp  shared(x2bjump,it,descent,mysectjumplist,nbody,fill,restrictjumps,compactjumpstorage,hspsqn,jumpcount) &
!$omp schedule(static)
...
!			 if(it==2)print*,' sector jumps counts ',isjmp, x2bjump(it)%isector(isjmp), x2bjump(it)%fsector(isjmp), & 
!			   x2bjump(it)%sjmp(isjmp)%njumps
             jumpcount(isjmp) = njumps
     end if
...
!......... ADD UP JUMPS..............
  ...
  if(.not.fill)then 
     if(it == 1)then
        ...
        if ( iproc == 0 .and. verbose_jumps ) write(6,*)totjumps,' proton jumps' 
        if ( iproc == 0  ) write(logfile,*)totjumps,' proton 2b jumps' 
		
     else
        totn2bjumps = totjumps
        if ( iproc == 0 .and. verbose_jumps) write(6,*)totjumps,' neutron jumps' 
        if ( iproc == 0  ) write(logfile,*)totjumps,' neutron 2b jumps' 
...
subroutine set2bsectorjumps(it,create,hermflag,whermflag,changeq,changew,ndm )
   ...
   do is = 1,nsectors(it)
     ...
     iw  = xsd(it)%sector(is)%wX
     do fs = 1,nsectors(it)
        !----------- CHECK IF A TWO-BODY OPERATOR CAN EVEN REACH-----------
        if( abs(ijz - xsd(it)%sector(fs)%jzX) > maxdm)cycle
...
!		if(.not.create .and. it==2)print*,it,is,fs, 'sector jump',nsjmps
...
!	  if(it==2)then
!		  print*,' CHECKING!!! ',it
!		  print*,nsjmps,hermflag,whermflag,changeq,changew
!	  end if
...
subroutine split_sectorjumps(Nbody,it)
	...
	logical :: skipme = .true.  ! for testing purposes only
...
!..................... FIND SPLITS AND COPY OVER TO TEMPORARY .........................	
	...
	do isj = 1,xNbjump%nsectjumps
		...
		if(localjumps > maxjumpsallowed)then    ! RECORD SPLIT
...
!		print*,it,' max # ',Nbody,'-body jumps (in a sector jump) ',maxjumpsfound
	...
	if(nsectjumps0 == nsectjumpsX .or. skipme)then  ! NO PROBLEM, DO NOT SPLIT
...
! .... TESTING ONLY...
		if(split(isj)>0 .and. iproc==0)then
			print*,' checking: ',xtmpjump%sjmp(isj)%nstart,xtmpjump%sjmp(isj)%nstart+xtmpjump%sjmp(isj)%njumps-1,stride
			do k = split(isj),0,-1
				print*,xNbjump%sjmp(jsj-k)%nstart,xNbjump%sjmp(jsj-k)%nstart+xNbjump%sjmp(jsj-k)%njumps-1
			end do
		end if
