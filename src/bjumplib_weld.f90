...
!		 write(27,*)njumps,rcurch%chain%oplist	 
         select case (nbody)
...
!-------------ONE-BODY  --- a^+i a_j
          ...
          if(i < 0)then
           ...
           if( fill .and. njumps >= minjump .and. njumps <=maxjump ) then
              ...
              phase(njumps-jumpshift)= int(lphase*rphase*genphase*myTRphase, kind=1)
           endif
...
!		  if(njumps > 0)then
!			  write(51,'(9i4)')njumps,i,j,k,l,hspsqn(it,i)%j,hspsqn(it,j)%j,hspsqn(it,k)%j,hspsqn(it,l)%j
!		  end if
		  if(i==0 .or. j==0 .or. j==0 .or. k==0)then
		  	print*,' some strange error ',i,j,k,l
		  	stop
		  end if
...
!
          ...
          if(useTRnew)then
              ...
              if(j < 0) then
                 mm = mm+hspsqn(-it,-j)%m
 				jsumphase = jsumphase + hspsqn(-it,-j)%j
			  ...
			  if(mm < 0)then
	              ...
	              if( k < 0 )then
					jsumphase = jsumphase + hspsqn(-it,-k)%j
					k=hspsqn(-it,-k)%tr  ! map to time-reversed state
	              ...
	              if(l < 0) then
	 				jsumphase = jsumphase + hspsqn(-it,-l)%j
					l=hspsqn(-it,-l)%tr
				  ...
				  myTRphase = (-1)**( jsumphase/2  )
!........... SEND TO THEIR CONJUGATES...
...
!----------- THIS PHASES MAY BE BACKWARDS
          ...
          if(dpair <= 0)then
              print*,' dpair ',dpair,k,l,ksps,lsps
			  print*,k+nhsps(-it)
              print*,opsum
              stop
          endif
...
!------------------ FIND M, PAR OF OPERATORS
          ...
          if(first)then
		   ...
		   if(useTRnew .and. m < 0)then
			   print*,' should not have gotten here ',m
			   print*,i,j,k,l
			   stop
		   end if
...
!------------------ OPTION TO SKIP OVER ZERO MATRIX ELEMENT JUMPS ----------
           ...
           if(fill .and. njumps-xjump%nstart > xjump%njumps)then
			   ...
			   call BMPI_Abort(MPI_COMM_WORLD,333,ierr)
#endif
              stop
            endif
            if(fill .and. njumps >= minjump .and. njumps <=maxjump )then
!				print*,' test in weld ',njumps,itbme,cpair,dpair,isd,fsd, xxstart(isd),xxstart(fsd)!cpair0,dpair,dpair0,isps,jsps,ksps,lsps
...
!					   if(itbme==2)print*,cpair,dpair,' help ',njumps-jumpshift,i,j,k,l,isd,fsd
