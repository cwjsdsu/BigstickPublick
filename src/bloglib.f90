...
1001 continue
    ...
    write(logfile,*)' Lanczos option ',lanczchar
    select case (lanczchar)
       case ('ld')
	     write(logfile,*)' Default Lanczos'
 	     write(logfile,*)nkeep,' vectors kept from ',niter,' max iterations '
		 write(logfile,*)' '

 	  case ('lf')
             write(logfile,*)' Fixed iteration Lanczos'
    	     write(logfile,*)nkeep,' vectors kept from ',niter,' iterations (fixed)'
			 write(logfile,*)' '
			 
	  case('bd')
             write(logfile,*)' Default block Lanczos'
   	         write(logfile,*)nkeep,' vectors kept with block size ',dimblock, & 
			 ' with max ',maxblockiter,' block-iterations for total ',niter,' equivalent iterations'
			 write(logfile,*)' '
			 
 	  case ('td')
             write(logfile,*)' Default thick-restart Lanczos' 
   	         write(logfile,*)nkeep,' vectors kept with a max of ',niter,' iterations '
             write(logfile,*)' keeping ',nthick_keep,' vectors after a restart,', &
			     ' with a maximum of ',max_num_thick_restart,' restarts '
				 write(logfile,*)' '
			   
	  case ('tb') ! NOTE THIS NEEDS TO BE FINISHED
             write(logfile,*)' Thick-restart block Lanczos'
   	         write(logfile,*)nkeep,' vectors kept with block size ',dimblock,', with a maximum of ',&
			 maxblockiter,' block iterations '
			 write(logfile,*)' '
			 
	  case('sk')
             write(logfile,*)' Skipping diagonalization '
			 write(logfile,*)' '
