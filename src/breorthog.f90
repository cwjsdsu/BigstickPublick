...
#ifdef _MPI
   call BMPI_COMM_SPLIT(MPI_COMM_WORLD, color, key, br_hcomm, ierr)
...
subroutine setup_mpi_hist(maxiter)
   ...
   integer(kind=basis_prec) ::  csize(nfragments)       ! slice size for fragment
   integer ::  whichfrag(0:nproc)    ! assignment for processor
   integer(kind=basis_prec) :: bestfrag,i
   real(kind=8) :: g, bestg
...
!  if(iproc == 0) print *, "In setup_hist"
   ...
   ! FIXED in 7.11.2 for rare cases where a fragment size requires int*8
   csize =  int(((basestop - basestart + 1) + piinfragcnt - 1) / piinfragcnt,8)
   do pi =0,nproc-1
      fi = whichfrag(pi)
      if(fi == 0) then
...
#ifdef _MPI   
      call BMPI_ALLREDUCE(dp, tdp, 1, MPI_SUM, MPI_COMM_WORLD, ierr)
#else
      tdp=dp
#endif
   a = sqrt(tdp)
...
#ifdef _MPI
   call BMPI_ALLREDUCE(br_orthogdot, br_orthogdotf, maxhi, MPI_SUM, MPI_COMM_WORLD, ierr)
#else
   br_orthogdotf = br_orthogdot   
#endif
...
!  ADDED IN 7.11.3 for option 'jp'
!
!
! Remove overlap with the history buffer
! off supports ending lopo at br_histpos-1 when we have
! already removed overlap with the previous vector to get \alpha
! MODIFIED: ONLY FOR A RANGE
!
!  CALLED BY: lanczos_p
!
!  CALLS:
...
!  USES: MPI_ALLREDUCE
!
subroutine br_orthogonalize_restricted(ivecstart, off)
   use bmpi_mod
   implicit none
   integer :: off
   integer :: ierr
   integer :: hi, maxhi,ivecstart
   integer(kind=basis_prec) :: istate
   real(kind=8) :: dp, tdp, sum, m8
   real(kind=lanc_prec) :: m
...
!$omp end parallel do
end subroutine br_orthogonalize_restricted
...
#ifdef _MPI	   
      call BMPI_GATHERV(br_reg, count, buf, sendcounts, displs, 0, MPI_COMM_WORLD, ierr)
#endif
   end if
...
subroutine br_transform_basis(eiglvec, nkeep, eign)
   ...
   tmpvec = 0.0
   do i = ostart, ostop
!$OMP parallel do private(dtmp,j), shared(tmpvec)	   
...
! ADDED IN 7.11.3 FOR OPTION 'jp'
! transforms only a fixed range of vectors
!
!  eiglvec is a dim_all x dim_all matrix 
!  this transforms vectors from ivecstart to ivecstor
subroutine br_transform_basis_restricted(dim_all,eiglvec,ivecstart,ivecstop)
   implicit none
!   integer, intent(in) :: nkeep
   integer, intent(in) :: dim_all,ivecstart,ivecstop
   ! first index in eiglvec is vector member, second identifies the vector
   real(kind=egv_prec) :: eiglvec(dim_all,dim_all)
   real(kind=egv_prec) :: dtmp
   real(kind=lanc_prec) :: tmpvec(dim_all)
   integer :: pos, j
   integer(kind=basis_prec) :: i

   call br_check_setup()
   
   if(ivecstop - ivecstart+1 > dim_all)then
	   print*,' Some error in using br_transform_basis_restricted '
	   print*,ivecstart,ivecstop,dim_all
	   stop 233
   end if
! NEED TO FIX THIS ERROR TRAP   
!   if(nkeep > br_histpos .or. eign > br_histpos) then
!      if(iproc == 0) print *, "br_transform_basis: nkeep > br_histpos",nkeep,eign,br_histpos
 !     stop 23
!   end if
   ...
   do i = ostart, ostop
!$OMP parallel do private(dtmp,j), shared(tmpvec)	   
      do pos = ivecstart,ivecstop  ! for each output vector we want
         dtmp = 0.d0
         do j = ivecstart,ivecstop
            dtmp = dtmp + real(br_histbuf(i, j),kind=8) * real(eiglvec(j+1-ivecstart, pos+1-ivecstart), kind=8)
         end do
         tmpvec(pos+1-ivecstart) = real(dtmp, kind=lanc_prec) ! dtmp is higher precision so sum is accurate
      end do

      ! now replace the column in the history buffer
      do pos = ivecstart,ivecstop
         br_histbuf(i, pos) = tmpvec(pos+1-ivecstart)
      end do
   end do
end subroutine br_transform_basis_restricted
...
!...... FIND OUT WHERE THE COEFFICIENTS ORIGINATE FROM.....		
		fromproc = -1
		do i = 0,nproc -1
		    if(pos >= br_ostartlist(i) .and. pos <= br_ostoplist(i) .and. isfragroot)then
				if(fromproc >= 0)then
	    ...
	    if(pos >= ostart .and. pos <= ostop )then
			do i = 1,nkeep
	 		   vamp(i) = br_histbuf(pos, i)
			   tmpvamp(i)=vamp(i)
			end do	
			if(iproc/=fromproc)print*,' MISMATCH IN fromproc ',fromproc,iproc
			   ...
			   tag = iproc
#ifdef _MPI			   
			   if(fromproc > 0)call BMPI_SEND(tmpvamp,nkeep,0,tag,MPI_COMM_WORLD,ierr)
#endif
 	    end if

		if(iproc==0 .and. fromproc > 0)then
			if(fromproc /= 0)then
 			   tag = fromproc
#ifdef _MPI			   
 			   call BMPI_RECV(vamp,nkeep,fromproc,tag,MPI_COMM_WORLD,stat,ierr)
#endif			   
	         end if
		end if
...
! ........ ADDED 7.10.7 by CWJ.......
! pushes coefficients for a specific basis label

subroutine br_push_coef(pos,nkeep,vamp)
	use precisions
	use localvectors
	use nodeinfo
	use bmpi_mod
...
!	call br_check_setup()
	...
	if(nproc==1)then
		do i = 1,nkeep
 		   br_histbuf(pos, i) = vamp(i)
		end do

		return
	else
		
!...... FIND OUT WHERE THE COEFFICIENTS GO TO .....		
		destproc = -1
		do i = 0,nproc -1  
		    if(pos >= br_ostartlist(i) .and. pos <= br_ostoplist(i) .and. isfragroot)then
				if(destproc >= 0)then
...
#ifdef _MPI		
		if(iproc==0)then  ! cooy to tmp array
 		  tmpvamp = vamp
		end if
		tag = destproc
		if(iproc==0)then ! send from root to destination fromproc
		   call BMPI_SEND(tmpvamp,nkeep,destproc,tag,MPI_COMM_WORLD,ierr)
		end if  
		if(destproc==iproc)then ! receive from root to this proc
		   tag = destproc		   
		   call BMPI_RECV(vamp,nkeep,0,tag,MPI_COMM_WORLD,stat,ierr)
		   vamp=tmpvamp
	    end if
#endif				
	    if(pos >= ostart .and. pos <= ostop )then
			do i = 1,nkeep
	 		   br_histbuf(pos, i) = vamp(i) ! write to file
			end do	
		end if
