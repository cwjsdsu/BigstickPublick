...
!------------------OPEN A FILE---------------------------------------------- 
  ...
  do while ( .not. success ) 
     ...
     call BMPI_BCAST(filename,LEN(filename),0,MPI_COMM_WORLD,ierr) 
#endif
     ilast = index(filename,' ') - 1 
...
#ifdef _MPI
     call BMPI_BCAST(success,1,0,MPI_COMM_WORLD,ierr) 
     call BMPI_BCAST(nushellsp,1,0,MPI_COMM_WORLD,ierr) 
#endif
     if(nushellsp)then
...
#ifdef _MPI
        call BMPI_BCAST(orbqn(1,i)%nr,1,0,MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(orbqn(1,i)%l,1,0,MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(orbqn(1,i)%par,1,0,MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(orbqn(1,i)%j,1,0,MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(orbqn(1,i)%w,1,0,MPI_COMM_WORLD,ierr) 
#endif
     end do 
     numorb(2) = numorb(1) 
     orbqn(2,:) = orbqn(1,:) 
...
#ifdef _MPI
           call BMPI_BCAST(orbqn(2,i)%nr,1,0,MPI_COMM_WORLD,ierr) 
           call BMPI_BCAST(orbqn(2,i)%l,1,0,MPI_COMM_WORLD,ierr) 
           call BMPI_BCAST(orbqn(2,i)%par,1,0,MPI_COMM_WORLD,ierr) 
           call BMPI_BCAST(orbqn(2,i)%j,1,0,MPI_COMM_WORLD,ierr) 
           call BMPI_BCAST(orbqn(2,i)%w,1,0,MPI_COMM_WORLD,ierr) 
#endif
        end do 
	else !wcolumn must = 2
        do i = 1, numorb(1) 
...
!--- added in 7.10.7 -- check for negative W and shift if necessary  
  ...
  do it = 1,2
	  do i = 1,numorb(it)
		  wneg = min(wneg,orbqn(it,i)%w)
	  end do
  end do
  if(wneg < 0)then ! shift
	  if(iproc==0)then
		  print*,' Lowest W value is negative ',wneg,'; shifting'
	  end if
	  do it = 1,2
		  do i = 1,numorb(it)
			  orbqn(it,i)%w=orbqn(it,i)%w - wneg
		  end do
	  end do 
	  
!	  print*,' Neutron Ws ',orbqn(2,:)%w
...
1001 continue
	...
	if(success)then
        ...
        call BMPI_BARRIER(MPI_COMM_WORLD,ierr) 
        call BMPI_BCAST(numorb(1),1,0,MPI_COMM_WORLD,ierr) 	
		call BMPI_BCAST(nspecies,1,0,MPI_COMM_WORLD,ierr)
#endif
		if(nspecies==2)then
#ifdef _MPI
	        call BMPI_BCAST(numorb(2),1,0,MPI_COMM_WORLD,ierr) 
		...
		do it = 1,2
		  do iorb = 1,numorbmax
#ifdef _MPI
			  call BMPI_BCAST(orbqn(it,iorb)%nr,1,0,MPI_COMM_WORLD,ierr)
			  call BMPI_BCAST(orbqn(it,iorb)%l,1,0,MPI_COMM_WORLD,ierr)
			  call BMPI_BCAST(orbqn(it,iorb)%j,1,0,MPI_COMM_WORLD,ierr)
			  call BMPI_BCAST(orbqn(it,iorb)%w,1,0,MPI_COMM_WORLD,ierr)
#endif
...
#ifdef _MPI
  call BMPI_BARRIER(MPI_COMM_WORLD,ierr) 
  call BMPI_BCAST(nprinc,1,0,MPI_COMM_WORLD,ierr) 
#endif 
...
#ifdef _MPI
     call BMPI_BARRIER(MPI_COMM_WORLD,ierr) 
     call BMPI_BCAST(np(:),2,0,MPI_COMM_WORLD,ierr)  
#endif
  end if 
...
!.................. CHECK FOR M-value IF CHOOSING OPTION 'do', 'dxo'	 
 
     if(only_odd)then
		 if ((jz/2)*2 /= jz)then
			 if(iproc==0)then
				 print*,' This option only for even-A nuclides'
			 
			 end if
#ifdef _MPI
			 call BMPI_Abort(MPI_COMM_WORLD,910,ierr)
#endif
			 stop
		 end if
		 ...
		 if (abs(jz)==0)then
			 if(iproc==0)then
				 print*,' This option only for Jz /= 0'
			 
			 end if
#ifdef _MPI
			 call BMPI_Abort(MPI_COMM_WORLD,912,ierr)
			 stop
#endif
		 end if		 
	 end if
